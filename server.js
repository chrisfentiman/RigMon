      ...
      for (var j in ison) {
        if (ison[j] != oldon[j]) {
          if (ison[j]) {
            ...
            msgtext =
              msgtext + all[j].name + " is offline, e:" + all[j].error + "\n";
          }
          any = true;
        }
      }
...
bot.onText(/\/id/, msg => {
  bot.sendMessage(
    msg.chat.id,
    (text = "<pre>Your chat ID is: " + msg.chat.id + "</pre>"),
    { parse_mode: "HTML" }
  );
});
...
bot.onText(/\/status/, msg => {
  ...
  for (var f in rigs) {
    ...
    if (rig.error === null) {
      msgtext = msgtext + "Total Speed: ";
      time = moment.unix(rig.uptime);
      up = moment(time).toNow(true);
      if (rig.hash2 > 0) {
        msgtext =
          msgtext +
          (rig.hash / 1000).toFixed(2) +
          " MH/s, " +
          (rig.hash2 / 1000).toFixed(2) +
          "MH/s, ";
      } else {
        if (rig.hash > 10000) {
          msgtext = msgtext + (rig.hash / 1000).toFixed(2) + " MH/s, ";
        } else {
...
bot.onText(/\/profit/, msg => {
    ...
    if (hash[f] > 0) {
      if (f == 0)
        msgtext =
          ...
          power[f] +
          "W, " +
          ((hash[f] * 1000) / power[f]).toFixed(2) +
          "H/w\n" +
          "Power: d:" +
          ((power[f] * kwprice * 24) / 1000).toFixed(2) +
          e +
          " w:" +
          ((power[f] * kwprice * 24 * 7) / 1000).toFixed(2) +
          e +
          " m:" +
          ((power[f] * kwprice * 24 * 30) / 1000).toFixed(2) +
          e +
          "\n";
      if (f == 1 || f == 2)
        msgtext =
  ...
  msgtext =
    msgtext + "Total expenses for power: " + allpower.toFixed(2) + "kW\n";
  msgtext =
    msgtext +
    "d:" +
    (allpower * kwprice * 24).toFixed(2) +
    e +
    " w:" +
    (allpower * kwprice * 24 * 7).toFixed(2) +
    e +
    " m:" +
    (allpower * kwprice * 24 * 30).toFixed(2) +
    e +
    "\n";
          ...
          .get(coinpath, res => {
            ...
            res.on("end", () => {
              ...
              for (var key in profits["coins"]) {
                ...
                if (profit.algorithm == "Ethash")
                  ethtext =
                    ethtext +
                    pr +
                    " | d:" +
                    (profit.btc_revenue24 * btc).toFixed(2) +
                    e +
                    ", w:" +
                    (profit.btc_revenue24 * btc * 7).toFixed(0) +
                    e +
                    ", m:" +
                    (profit.btc_revenue24 * btc * 30).toFixed(0) +
                    e +
                    "\n";
                if (profit.algorithm == "Equihash")
                  eqtext =
                    eqtext +
                    pr +
                    " | d:" +
                    ...
                    e +
                    "\n";
                if (profit.algorithm == "CryptoNight")
                  cntext =
                    cntext +
                    pr +
                    " | d:" +
              ...
              msgtext =
                msgtext +
                "ZEC+ETH+XMR-Electricity:\nd:" +
                dayprofit.toFixed(2) +
                e +
                ", w:" +
                (dayprofit * 7).toFixed(2) +
                e +
                "\nm:" +
                (dayprofit * 30).toFixed(2) +
                e +
                ", y:" +
                (dayprofit * 365).toFixed(2) +
                e +
                "</pre>";
              bot.sendMessage(msg.chat.id, (text = msgtext), {
                parse_mode: "HTML"
              });
            });
          })
...
bot.onText(/\/detailed/, msg => {
  ...
  for (var f in rigs) {
    ...
    if (rig.error == null) {
      msgtext =
        msgtext + " | " + (rig.hash / rig.power).toFixed(2) + "H/W" + "\n";
...
bot.onText(/\/history/, msg => {
  ...
  bittrex.getwithdrawalhistory({}, function(data, err) {
    ...
    bittrex.getdeposithistory({}, function(data, err) {
      ...
      bot.sendMessage(msg.chat.id, (text = msgtext + "</pre>"), {
        parse_mode: "HTML"
      });
    });
  });
  poloniex.returnDepositsWithdrawals(timeLastWeek, timeNow, function(
    err,
    data
  ) {
    if (err) {
